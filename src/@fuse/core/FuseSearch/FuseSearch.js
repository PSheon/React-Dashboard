			...
			{variant === 'basic' ? (
				...
				<TextField
					...
					InputProps={{
						...
						classes: {
							input: clsx(classes.input, 'py-0 px-16 h-64 rounded-l-12')
						}
					}}
					variant="standard"
					{...other}
...
function renderSuggestion(suggestion, { query, isHighlighted }) {
	...
	return (
		<MenuItem
			...
			classes={{ root: 'transition-all duration-300 ease-in-out hover:ml-12' }}
		>
							...
							>
								<div ref={suggestionsNode}>
									...
									>
										{options.children}
										{state.noSuggestions && (
											<Typography className="px-16 py-12" variant="caption" color="textSecondary">
												<Icon>close</Icon>
												沒有相關結果
											</Typography>
										)}
		...
		case 'full': {
			return (
				<div className={clsx(classes.root, 'flex', props.className)}>
					<Tooltip title="尋找頁面" placement="bottom" className="flex justify-between items-center h-64">
					...
					{state.opened && (
						<ClickAwayListener onClickAway={handleClickAway}>
							<Paper className="absolute left-0 right-0 h-full z-9999 rounded-12" square>
								<div className="flex items-center w-full" ref={popperNode}>
									<Icon className="w-48 h-48 pl-20 py-12">search</Icon>
									<Autosuggest
										{...autosuggestProps}
										inputProps={{
											classes,
											...
											>
												<div ref={suggestionsNode}>
													<Paper
														...
														square
														{...options.containerProps}
														className="pt-12 py-16 rounded-12"
														style={{
													...
													>
														...
														{state.noSuggestions && (
															<Typography
																className="px-16 py-12 flex items-center"
																variant="subtitle1"
																color="textSecondary"
															>
																<Icon className="mx-12">error_outline</Icon>
																沒有結果
															</Typography>
														)}
													</Paper>
...
FuseSearch.defaultProps = {
	trigger: (
		<IconButton className="w-48 h-48 -mr-8 sm:-mr-4">
			<Icon>search</Icon>
		</IconButton>
	),
