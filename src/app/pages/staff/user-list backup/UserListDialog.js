...
function UserListDialog() {
	...
	function handleAdminResetPassword(email, password) {
		setShowNewPasswordHint(false);
	...
	//   dispatch(
	//     Actions.toggleUserActivation({ userId: id, email, active: !active })
	//   );
	//   closeComposeDialog();
	// }
	function handleDeleteUser() {
		// const { id, email, active } = form;
		const { id } = form;
		dispatch(Actions.deleteUser({ userId: id }));
		closeComposeDialog();
	}
			...
			<AppBar position="static" elevation={1}>
				...
				<div className="flex flex-col items-center justify-center pb-24">
					<Avatar
						...
						src={avatarNameToPathConverter(form.photoURL)}
					/>
			...
			<form noValidate onSubmit={handleSubmit} className="flex flex-col md:overflow-hidden">
				<DialogContent classes={{ root: clsx(classes.scrollbarWrapper, 'p-24') }}>
                  ...
                  員工
								...
								<MenuItem className="rounded-full" value="admin">
									管理員
								</MenuItem>
							</CssTextField>
						</div>
            ...
            {form.active ? (
              ...
              <Icon className="text-red">lock</Icon>
					...
					<IconButton onClick={handleDeleteUser}>
						<Icon className="text-red">lock</Icon> 刪除使用者
					</IconButton>
					{isAdmin && (
						<Button
